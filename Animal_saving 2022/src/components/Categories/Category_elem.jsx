...
import Vide from "../Main/Vide/Vide";
...
const Category_elem = (props) => {
    const category_block = useRef(null);
    const [isclose, setIsclose] = useState(false);
    let Vide_item = props.categories[props.elem].videos.map( function(m, index){return( <Vide category={props.categories[props.elem].name} animal={m.animal} SETCURRENT={props.SETCURRENT_CHATMODELIDAC} video_name={m.video_name} cover_img={m.cover_img} index={index} key ={`${index}_animal_${m.id}`} value={m} country={m.country} name={m.name} lovense_key={m.lovense_key} device={m.device} cover={m.cover}/>) });
        ...
        <div style={{ marginLeft: "auto", marginRight: "auto"}}>
            <div className={classes.category_block} id="ct_block">
                <a className={classes.category_block_a}>
                    {props.categories[props.elem].name}
                </a>
