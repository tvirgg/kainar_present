...
import {SETCURRENT_CHATMODELIDAC, SETCURRENT_VideowatchAC} from "../../redux/state/Broadcast_reducer";
...
const Main = (props) => {
    ...
    let Vide_item = props.videos.map( function(m, index){return( <Vide SETCURRENT_VideowatchAC={props.SETCURRENT_VideowatchAC} SETCURRENT={props.SETCURRENT_CHATMODELIDAC} video_name={m.video_name} index={index} key ={`${index}_animal_${m.id}`} value={m} country={m.country} name={m.name} lovense_key={m.lovense_key} device={m.device} cover={m.cover}/>) });
    return (
